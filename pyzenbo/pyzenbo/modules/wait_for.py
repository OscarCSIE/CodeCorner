...
class WaitForListenDoa(BaseWaitFor):

    def __init__(self, inter_comm):
        super().__init__(inter_comm)
        self.event_doa = threading.Event()
        self.doa_result = None

    def _listen_handler(self, args):
        logging.debug('WaitForListenDoa_listen_handler: %s', args)
        event_vad = args.get('event_vad_status', None)
        if event_vad:
            vad_status = event_vad.get('vad_status', None)
            logging.debug('WaitForListenDoa vad_status: %s', vad_status)
            if vad_status == 'onDOA':
                self.doa_result = event_vad
                self.event_doa.set()
        slu = args.get('event_slu_query', None)
        if slu:
            utterance = slu.get('user_utterance', None)
            logging.debug('WaitForListenDoa_slu_query: %s',
                          slu.get('error_code'))
            if utterance and slu.get('error_code') != 'success':
                self.doa_result = None
                self.event_doa.set()

    def start(self, timeout):
        self.event_doa.wait(timeout)
        self._restore_callback()
        return self.doa_result
