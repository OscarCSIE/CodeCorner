...
class Motion:
    ...
    def get_avoidance_status(self, sync=True, timeout=None):
        ...
        :param sync: True if this command is blocking, defaults to True
        :param timeout: maximum blocking time in second, None means infinity,   defaults to None
        :return: serial number of the command, if command is blocking also
            return a dict, it include two key, 'state' indicate execute
            result and 'error' will contain error code
        """
        des = DESTINATION["coordinator"]
        cmd = commands.MOTION_GET_AVOIDANCE
        data = {}
        serial, error = self._inter_comm.send_command(des, cmd, data, sync,
                                                      timeout)
        return serial, error

    def set_avoidance_status(self, enable, sync=True, timeout=None):
        """
        Turn on/off the obstacle-avoidance.

        :param enable: flag to enable/disable obstacle-avoidance
        :param sync: True if this command is blocking
        :param timeout: maximum blocking time in second, None means infinity
        :return: serial number of the command, if command is blocking also
            return a dict, it include two key, 'state' indicate execute
        ...
        """
        des = DESTINATION["coordinator"]
        cmd = commands.MOTION_SET_AVOIDANCE
        data = {'setting': str(enable).upper()}
        serial, error = self._inter_comm.send_command(des, cmd, data, sync,
                                                      timeout)
        return serial, error
